...
motCommands = [0]; %command steer angle rate (delta dot) of front motor
 ...
 while (count < numTimeSteps)    
   [zdot, u] = rhs(currentState,p,K); %calculate derivatives based on EOM
   ...
   currentState(1,2:end) = previousState(1,2:end) + zdot*timestep; 
 ...
 if graph == 1
       animateBike(allStates,p,motCommands);
